...
export class SupabaseStorage implements IStorage {
  ...
  async createUser(user: InsertUser): Promise<User> {
    try {
      ...
      if (existingUser) {
        log(`User with email ${user.email} already exists. Returning existing user.`, 'storage');
        
        // If the existing user has a different ID, update their ID to match the auth user
        if (user.id && existingUser.id !== user.id) {
          log(`Existing user has different ID (${existingUser.id}) than auth user (${user.id}). Will return existing user.`, 'storage');
        }
      ...
      log(`Creating new user with data: ${JSON.stringify(user)}`, 'storage');
      ...
      if (user.id) {
        ...
        log(`Including id in user creation: ${user.id}`, 'storage');
      } else {
        log(`WARNING: No user ID provided for new user with email ${user.email}`, 'storage');
      }
      ...
      if (error) {
        log(`Error creating user: ${error?.message}`, 'storage');
        throw new Error(error?.message || 'Failed to create user');
      }
      
      if (!data) {
        log(`No data returned when creating user`, 'storage');
        throw new Error('Failed to create user - no data returned');
      }
      
      log(`User created successfully: ${JSON.stringify(data)}`, 'storage');
      return data as User;
    } catch (error) {
      log(`Exception in createUser: ${error}`, 'storage');
      throw error;
    }
  }
  ...
  async getDietaryProfileByUserId(userId: string): Promise<DietaryProfile | undefined> {
    console.log("Getting dietary profile for user ID:", userId, "Type:", typeof userId);
  ...
  async createDietaryProfile(profile: InsertDietaryProfile): Promise<DietaryProfile> {
    console.log("Creating dietary profile with user ID:", profile.userId, "Type:", typeof profile.userId);
  ...
  async getRecentFoodScans(userId: number | string, limit: number): Promise<FoodScan[]> {
    const { data, error } = await supabase
      .from('food_scans')
      .select('*')
      .eq('user_id', userId)
    ...
    try {
      ...
      if (error && error.code !== 'PGRST116') { // Not "no rows returned" error
        ...
        if (fallbackError && fallbackError.code !== 'PGRST116') {
          console.error("Error getting scan limit from original table:", fallbackError);
          return undefined;
        }
  ...
  async createScanLimit(limit: InsertScanLimit): Promise<ScanLimit> {
    try {
      ...
      if (error) {
        console.error("Error creating scan limit in v2 table, trying fallback:", error);
        ...
        if (fallbackError) {
          log(`Error creating scan limit: ${fallbackError?.message}`, 'storage');
          throw new Error(fallbackError?.message || 'Failed to create scan limit');
        }
  ...
  async updateScanLimit(limit: ScanLimit): Promise<ScanLimit> {
    try {
      ...
      if (error && error.code !== 'PGRST116') { // Not "no rows returned" error
        console.log("Error updating scan limit in v2 table, trying fallback:", error);
        ...
        if (fallbackError) {
          log(`Error updating scan limit: ${fallbackError?.message}`, 'storage');
          throw new Error(fallbackError?.message || 'Failed to update scan limit');
        }
  ...
  async incrementScanCount(userId: string): Promise<ScanLimit> {
    try {
      ...
      if (error && error.code !== 'PGRST116') { // Not "no rows returned" error
        console.log("Error incrementing scan count in v2 table, trying fallback:", error);
        ...
        if (fallbackError) {
          log(`Error incrementing scan count: ${fallbackError?.message}`, 'storage');
          throw new Error(fallbackError?.message || 'Failed to increment scan count');
        }

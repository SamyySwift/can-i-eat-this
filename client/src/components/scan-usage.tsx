...
export default function ScanUsage({ userId }: ScanUsageProps) {
  ...
  const { data: scanLimit, isLoading, isError } = useQuery<ScanLimit>({
  ...
  if (isError || !scanLimit) {
    return (
      <div className="p-4 border border-red-200 bg-red-50 rounded-lg">
        <p className="text-red-700">Failed to load usage information. Please try again later.</p>
      </div>
    );
  }
  ...
  const daysRemaining = Math.ceil((resetDateObj.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
      ...
      <Card>
        <CardContent className="p-6">
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-lg font-medium text-gray-900">Scans This Month</h3>
            <Camera className="h-6 w-6 text-primary" />
          </div>
          <div className="mt-2">
            <div className="flex items-baseline">
              <p className="text-3xl font-semibold text-gray-900">{scansUsed}</p>
            ...
            <p className="mt-1 text-xs text-gray-400">Resets in {daysRemaining} days</p>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="p-6">
          ...
          <div className="mt-2">
            <div className="grid grid-cols-3 gap-2 text-center">
              <div>
                <p className="text-3xl font-semibold text-green-500">{safetyCounts?.safe || 0}</p>
                <p className="text-xs text-gray-500">Safe</p>
              </div>
              <div>
                <p className="text-3xl font-semibold text-yellow-500">{safetyCounts?.caution || 0}</p>
                <p className="text-xs text-gray-500">Caution</p>
              </div>
              <div>
                <p className="text-3xl font-semibold text-red-500">{safetyCounts?.unsafe || 0}</p>

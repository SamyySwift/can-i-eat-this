    ...
    <div className="container mx-auto py-8 px-4">
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Your Dietary Profile</h1>
          <Button 
            variant="ghost" 
            size="sm" 
        ...
        {showExplainer && 
          <div className="mt-6 mb-8 animate-in fade-in-50 duration-300">
            <Alert variant="default" className="mb-6 bg-amber-50 border-amber-200">
              ...
              <AlertTitle className="text-amber-800">Understanding your dietary profile</AlertTitle>
              <AlertDescription className="text-amber-700">
                Your dietary profile helps us determine if foods are safe for you to eat. 
                Explore the guide below to understand different dietary restrictions.
              </AlertDescription>
            </Alert>
      ...
      <Tabs defaultValue="edit" className="w-full">
        <TabsList className="w-full max-w-md mb-6">
          <TabsTrigger value="edit" className="flex-1">Edit Your Profile</TabsTrigger>
          <TabsTrigger value="view" className="flex-1">View Active Restrictions</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="view" className="pt-2">
          <div className="glass rounded-xl shadow-lg overflow-hidden">
            <div className="px-6 py-8">
              <h2 className="text-xl font-bold mb-6">Your Active Dietary Restrictions</h2>
              ...
              <div className="grid gap-6 md:grid-cols-3">
                <div className="bg-red-50 rounded-lg p-4 border border-red-100">
                  ...
                  {existingProfile?.allergies && existingProfile.allergies.length > 0 ? (
                    <ul className="space-y-2">
                      {existingProfile.allergies.map((allergy: string) => (
                        <li key={allergy} className="flex items-center text-red-700">
                ...
                <div className="bg-green-50 rounded-lg p-4 border border-green-100">
                  ...
                  {existingProfile?.dietaryPreferences && existingProfile.dietaryPreferences.length > 0 ? (
                    <ul className="space-y-2">
                      {existingProfile.dietaryPreferences.map((preference: string) => (
                        <li key={preference} className="flex items-center text-green-700">
                          <span className="w-2 h-2 bg-green-500 rounded-full mr-2"></span>
                          <span className="capitalize">{preference.replace('-', ' ')}</span>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-gray-500 italic">No dietary preferences specified</p>
                  )}
                </div>
                ...
                <div className="bg-blue-50 rounded-lg p-4 border border-blue-100">
                  ...
                  {existingProfile?.healthRestrictions && existingProfile.healthRestrictions.length > 0 ? (
                    <ul className="space-y-2">
                      {existingProfile.healthRestrictions.map((restriction: string) => (
                        <li key={restriction} className="flex items-center text-blue-700">
                    ...
                    <p className="text-gray-500 italic">No health restrictions specified</p>
                  )}
                </div>
              </div>
            </div>
